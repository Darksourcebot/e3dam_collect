...
e3dam1 = """**
⚝ مرحبا بك في اوامر سي آر بـوينت
...
e3dam8 = """**مرحبا بك في قسم تحويل النقاط
...
e3dam9 = """**مرحبا بك في قسم تحويل النقاط
...
e3dam10 = """ze1 = TelegramClient(StringSession(session), api_id, api_hash)
...
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply(e3dam1)
    ...
    while running:  # التحقق من حالة التشغيل قبل كل تكرار
        try:
            ...
            if sender.id == ownerhson_id:
                ...
                joinu = await ze1(JoinChannelRequest('saythonh'))
                channel_entity = await ze1.get_entity(pot)
                await ze1.send_message(pot, '**جاري بدأ عملية التجميع بواسطة سي آر**')
                await ze1.send_message(pot, '/start')
                await asyncio.sleep(2)
                msg0 = await ze1.get_messages(pot, limit=1)
                await msg0[0].click(2)
...
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply(e3dam8)
...
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply(e3dam9)
